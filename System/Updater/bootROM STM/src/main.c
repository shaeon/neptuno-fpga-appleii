...
static const char * fn_firmware = "UPD_MC2PSTM";
...
//	DEBUG(1)
	...
	vdp_gotoxy(0, 6);
	vdp_prints("    Multicore 2+ STM updater    ");
	
	vdp_gotoxy(0, 10);
	vdp_prints("    Press ENTER to continue     ");
//	vdp_gotoxy(8, 11);
//	vdp_prints("and press ENTER");
	...
	vdp_gotoxy(0, 14);
	vdp_prints("        Are you sure you        ");
    vdp_prints("      want to update? (Y/N)     ");
	...
	//reset the STM 
    //here to release the SD card from boot menu
	REG_STM32_RESET = 1;
...
//	DEBUG(4)
	...
	{
			//             01234567890123456789012345678901
			display_error("Fail to open UPDATE file");
	}
...
/*	num_blocks = 0;
	...
	*/
	...
	//while(1)
	{
        ...
        vdp_gotoxy(0, 3);
        vdp_prints("Change the boot0 jumper: ");
        while (1)
        {      
    
            //reset the STM
            REG_STM32_RESET = 1;
            for (c=0; c< 15000; c++) __asm__("nop");	
            REG_STM32_RESET = 0;
