...
START:
	...
	res = f_open(&Fil, "UPDATE.STM", FA_READ);
	if (res != FR_OK) {
		if (error_count > 0) {
			goto START;
		}
		//             12345678901234567890123456789012
		display_error("Error opening UPDATE.STM file!");
	}
	...
	*/
	...
	//while(1)
	{
			...
			{	
				...
				if (res != FR_OK || bl != 512)
				{
					display_error("Fail to load a 512 bytes block");
				}
			...
			{	
				...
				for (i = 0; i < 2; i++)
				{
					...
					if (ret == 0)
					{
						...
						for (c = 0; c < 256; c++)
						{
							if (buffer_rd[ c ] != buffer [ c + ( i * 256 ) ] ) //peek(addr))
							{ 
								/*vdp_prints("C:"); 
								vdp_putchar_hex(c);
								vdp_prints(" I:");
